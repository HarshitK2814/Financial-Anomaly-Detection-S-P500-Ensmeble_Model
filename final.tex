...
% Interact-specific citation support
...
\author{
\name{Prashant Hemrajani\textsuperscript{a}\thanks{CONTACT Prashant Hemrajani. Email: prashant.hemrajani@jaipur.manipal.edu}, Harshit Kumar\textsuperscript{a}, Sarthak Kumar\textsuperscript{a}, Shashank Shekhar\textsuperscript{a}, Varsha Himthani\textsuperscript{b}, Sayan Mahalik\textsuperscript{a}, and Manoj Kumar Bohra\textsuperscript{a}}
\affil{\textsuperscript{a}Department of IoT and Intelligent Systems, Manipal University Jaipur, Jaipur, Rajasthan, India. Emails: prashant.hemrajani@jaipur.manipal.edu, harshit.23fe10cii00175@muj.manipal.edu, sarthak.23fe10cii00229@muj.manipal.edu, shashank.23fe10cii00220@muj.manipal.edu, sayanmahalik6@gmail.com; \textsuperscript{b}School of Computer Science Engineering and Technology, Bennett University, Greater Noida, Uttar Pradesh, India. Email: varsha.himthani@gmail.com}
}
...
non-Gaussian behavior, heteroskedasticity, and structure altering regimes~\cite{mandelbrot1963variation, cont2001empirical}. The volatility clustering, fat-tails, and
leverage effect destroy stationarity assumptions inherent in
conventional methods for anomaly detection~\cite{bouchaud2001more, fama1970efficient}. Identification of anomalies within such markets is far from being a
...
of detection during crisis~\cite{chandola2009anomaly}. Approaches using conditional
volatility such as GARCH~\cite{bollerslev1986a} and EGARCH~\cite{nelson1991egarch} partly resolve
the heteroskedasticity issue, as their initial purpose was volatility estimation~\cite{engle1982arch, hansen1994autoregressive}.

The advent of deep learning techniques has led to an era
of anomaly detectors built around autoencoders, variational autoencoders (VAEs), and transformers with good reconstruction capability within the sample~\cite{darban2023deep, pang2021deep}. Nevertheless, these methods share three major structural issues. First,
...
to the absolute value of the return, crucial in crash detection~\cite{crepey2022anoma}. Third, black box algorithms provide no justification for the economy behind their decisions, thus violating
Basel III/IV standards~\cite{arrieta2020explainable}. From the empirical side, deep
methods over-fit sample properties and show poor results over
time as structural breaks affect the data generation process~\cite{liu2024times}.

One common feature in all these issues is the lack of regime
structure explicitly introduced in the model. Returns distributions change depending on the volatility regimes~\cite{hamilton1989a, ang2002regime, guidolin2011markov},
...
economically grounded~\cite{jacobs1991moe, alkilane2024mixma}.
...
learning~\cite{liu2024times, chandola2009anomaly}. However, unlike generic anomaly detection, financial data have heteroskedasticity, regime changes, extremely imbalanced classes, and structural breaks. All four streams will be
described in turn.

\subsection{Statistical/Econometric Methods}

Initial attempts at anomaly detection based on parametric
models that assumed stationary distribution of returns~\cite{mandelbrot1963variation, cont2001empirical, fama1970efficient}.
...
crisis situation~\cite{engle1982arch, bollerslev1986a}.

Conditional volatility models such as GARCH~\cite{bollerslev1986a}, EGARCH~\cite{nelson1991egarch}
...
as the Markov Switching Models~\cite{hamilton1989a} and Hidden Markov Models,
have usually been used for identifying hidden regimes, such as bull
market regime, bear market regime, and crisis regimes~\cite{ang2002regime, guidolin2011markov}.
...
forecasting-related Mixture of Experts (MoE) models~\cite{alkilane2024mixma}
...
Fan and Li~\cite{fan2025expla}, where an adaptive MoE model employing a GNN-based
...
Anomaly detection using deep learning algorithms has progressed due to developments in representation learning~\cite{darban2023deep, pang2021deep}. Recurrent networks, convolutional networks, variational autoencoders, and transformers exhibit high in-sample performance on financial time-series data~\cite{su2025anoma, wu2024anoma, cao2024autom}. Three structural issues prevent them from being used effectively in financial applications. First, rare tail observations (about 2\%) cause very severe class imbalance. Second, normalization of inputs prevents them from detecting the actual magnitude of returns that is critical for crash detection. Third, deep learning algorithms typically learn noise rather than economic structure, especially in small samples. The development of attention mechanisms, ensembles, and focal loss has led to improvements in in-sample performance but failed to solve the interpretability problem, which is a hard constraint under Basel~III/IV~\cite{arrieta2020explainable}. Long-term forecast performance is poor because regimes shift invalidating in-sample patterns~\cite{liu2024times}.
...
preference~\cite{arrieta2020explainable}. Linear factor models, decision trees, and
shallow ensemble approaches~\cite{breiman2001random, friedman2001greedy} are still in use because
...
are successfully utilized to construct highly predictive input representations through factor-based representations. Instance-wise post-hoc explanations by means of the SHAP~\cite{lundberg2017shap}
and LIME~\cite{ribeiro2016lime} methods are gaining ground in finance~\cite{wang2024adapt},
...
Song \cite{song2023anoma} & 2023 & VAE-Transformer & DeFi & Recon\-struction & Short-Horizon OOS & No & Low & Single DeFi dataset; no regime analysis \\
...
Cao \& Guo \cite{cao2024autom} & 2024 & RL-based selection & SWaT/ COBT & Recon\-struction & Short-Horizon OOS & No & Low & Threshold-dependent; no financial regimes \\
Fan \& Li \cite{fan2025expla} & 2025 & Adaptive MoE + GNN & US equities & Local Deviation & Short-Horizon OOS & Yes & High & Fixed four-expert taxonomy; no VaR analysis \\
...
Su \cite{su2025anoma} & 2025 & WV+LSTM+\newline Transformer & S\&P/ Binance & Recon\-struction & Short-Horizon OOS & No & No & Static anomaly thresholds \\
Zhou et al.\ \cite{zhou2025kanad} & 2025 & KAN-AD (Fourier) & KPI/ SMAP & Recon\-struction & Short-Horizon OOS & No & Low & Univariate focus; no financial regimes \\
...
Pang et al.\ \cite{pang2021deep} & 2021 & Survey: Deep AD & Multi-domain & Academic Domain & Literature Review & No & No & No standardized financial evaluation; regime shifts ignored \\
Breiman \cite{breiman2001random} & 2001 & Random Forest & UCI / Finance & Global Outlier & Cross-Val OOB & No & Med & Static global threshold; no temporal or regime structure \\
Sch\"{o}lkopf et al.\ \cite{scholkopf2001estimating} & 2001 & One-Class SVM & UCI/ Synthetic & Density Support & In-Sample CV & No & Low & Kernel choice sensitive; quadratic training cost; no temporal dynamics \\
Engle \& Manganelli \cite{engle2004caviar} & 2004 & CAViaR Reg.\ Quantile & US/UK Equities & VaR Tail & Rolling OOS & Implicit & Low & Predicts conditional quantile only; no binary anomaly classification \\
Wu et al.\ \cite{wu2024anoma} & 2024 & RNN + Latent VAR & Financial TS & Recon\-struction & Short-Horizon OOS & No & No & Black-box; requires supervised labels; no regime conditioning \\
\bottomrule
\end{tabular}%
}}
...
concurrent studies, Fan and Li~\cite{fan2025expla} utilize an adaptive MoE
...
crash, and 2022 inflation-induced market drawdown~\cite{cont2001empirical}. The data was split
into non-overlapping windows of 128 days without shuffling. Since the 128
...
the magnitudes necessary for detecting extreme events~\cite{bouchaud2001more}.
...
accumulation~\cite{bouchaud2001more}. Both the features and labels used for training were computed based on the backward-looking data only,
...
However, financial markets exhibit heteroskedasticity and non-stationarity~\cite{bouchaud2001more, hamilton1989a},
...
latent states of volatility according to the Baum--Welch algorithm~\cite{hamilton1989a}. Transition probabilities and a regime persistence
...
be investigated using backward looking observation~\cite{cont2001empirical}. Each regime
...
clustered~\cite{bouchaud2001more}.
...
\end{equation}
which captures exhaustion in panic selloffs~\cite{cont2001empirical}.
...
\end{equation}
where $H_i$, $\mathrm{Lo}_i$, $O_i$, $C_i$ denote the high, low, open, and close within each sub-period. The Parkinson estimator \cite{parkinson1980extreme} provides a range-based alternative:
\begin{equation}
...
\end{equation}
reflecting the heavy-tailed nature of financial return distributions \cite{bouchaud2001more}, and skewness quantifies return asymmetry, with negative values indicating crash-prone left tails. The extended library adds realized skewness, a within-window complement computed directly from the return series, and maximum drawdown, which captures the worst-case loss within each window:
\begin{equation}
...
\end{equation}
capturing institutional flow anomalies. The Amihud illiquidity ratio \cite{amihud2002illiquidity} quantifies price impact per unit volume:
\begin{equation}
...
All deep learning experts are trained with a focal loss function \cite{lin2017focal} to address the severe class imbalance~\eqref{eq:focal}:
\begin{equation}
...
Evaluation distinguishes between two modes of assessment with distinct scientific objectives~\cite{liu2024times, chandola2009anomaly}.
...
\begin{enumerate}[label=\arabic*., itemsep=3pt]
    ...
    \item Event-F1 ($\mathcal{F}_{1}^{\mathrm{E}}$): F1 score with tolerance of $\pm3$ trading-days,
       adhering to the common practice for detection of sparse point events~\cite{liu2024times}.
\end{enumerate}
...
Figure~\ref{fig:cumret} shows the path taken by the cumulative returns over the 2022--2025 hedge period. The regime-aware RC-MoE strategy overlay (blue solid line) mimics the buy-and-hold in periods of up-trends but deviates from it defensively in times of drawdowns, and, as can be seen from the lower graph, which is cumulative returns relative to buy-and-hold, this results in risk reduction, as shown in the Fama-French analysis ($\beta=1$, $\alpha=0$) (Section~\ref{subsec:factor_exposure}).
...
In order to formally assess whether the regime-aware hedge overlay creates value in addition to passively being exposed to the known risk factors, we regress its daily abnormal return ($R_{\text{hedge},t} - R_{f,t}$) on the Fama-French six-factor model (including Momentum), formulated in~\eqref{eq:ff6}:
\begin{equation}
\begin{split}
...
Table~\ref{tab:var} demonstrates that unconditional tail-risk estimates systematically misrepresent capital requirements. All VaR and CVaR figures are computed directly from actual daily S\&P 500 returns partitioned by rolling 60-day realised volatility regimes~\cite{jorion2007value}; all values represent 1-day losses and are reported as negative numbers (e.g., $-1.67\%$ denotes a 1.67\% loss). CVaR (Expected Shortfall) is employed as a coherent risk measure~\cite{artzner1999coherent, rockafellar2000cvar} satisfying the properties of monotonicity, sub-additivity, homogeneity, and translational invariance.
...
regime~\cite{mcneil2000estimation}, using the $90$th loss percentile as the threshold $u$, and then backtest the resulting Value-at-Risk with the Kupiec proportion-of-failures
(POF) test~\cite{kupiec1995techniques} and the Christoffersen conditional-coverage
test~\cite{mcneil2015quantitative}.
...
the $\approx\!92\%$ estimate stated earlier, now grounded in a formal
...
\noindent where $d_{\tau}$ is the normalised Kendall tau distance~\cite{kendall1938measure} between feature-importance ranking vectors $\pi_i$ and $\pi_j$ from models $i$ and $j$, and $K = 3$. An ECI of 1 denotes perfect agreement; 0 denotes maximum disagreement.
...
The global ECI of 0.768 [0.685, 0.841], as shown in Fig.~\ref{fig:eci_regimes}, indicates strong cross-model agreement on anomaly drivers, substantially above the null baseline of 0.500 ($z=6.71$). All three model families independently identify composite volatility and GARCH-based features as top-2 drivers, consistent with the ARCH literature~\cite{bollerslev1986a} and the established role of volatility-of-volatility in tail risk~\cite{bouchaud2001more}.
...
The raw daily market series underlying this study (S\&P 500, NASDAQ, Russell 2000, and Bitcoin) are openly available via the open-source \texttt{yfinance} Python library \cite{yfinance} (\url{https://pypi.org/project/yfinance/}), and the complementary risk factor structures used for the Fama-French exposure and alpha verification models are openly available from the Kenneth R. French Data Library \cite{french_data} (\url{https://mba.tuck.dartmouth.edu/pages/faculty/ken.french/data_library.html}). The derived, localized rolling windows, engineered factor channels, and the precise values used to compute all reported means, standard deviations, tables, and figures that support the findings of this study are openly available in Zenodo at \url{https://doi.org/10.5281/zenodo.21257024} \cite{hemrajani2026data}, reference number zenodo.21257024, under a CC-BY 4.0 license. There are no datasets in this study that are restricted from sharing.

\section*{Code availability}
The code used for factor extraction, regime conditioning, model training, and evaluation is openly available in Zenodo at \url{https://doi.org/10.5281/zenodo.XXXXXXX} \cite{hemrajani2026code}, reference number zenodo.XXXXXXX, under a CC-BY 4.0 license. This archive is a permanent snapshot of the corresponding GitHub repository, \url{https://github.com/HarshitK2814/Financial-Anomaly-Detection-S-P500-Ensmeble_Model}.

\begin{thebibliography}{99}

\bibitem{alkilane2024mixma}
K.~Alkilane, Y.~He, and D.-H.~Lee, \emph{MixMamba: Time series modeling with adaptive expertise}, Information Fusion 112 (2024), p. 102589.

\bibitem{amihud2002illiquidity}
Y.~Amihud, \emph{Illiquidity and stock returns}, Journal of Financial Markets 5 (2002), pp. 31--56.

\bibitem{ang2002regime}
A.~Ang and G.~Bekaert, \emph{Regime switches in interest rates}, Journal of Business and Economic Statistics 20 (2002), pp. 163--182.

\bibitem{yfinance}
R.~Aroussi, \emph{yfinance: An open-source financial data downloader}, Python Package Index (2025), \url{https://pypi.org/project/yfinance/}.

\bibitem{artzner1999coherent}
P.~Artzner, F.~Delbaen, J.-M.~Eber, and D.~Heath, \emph{Coherent measures of risk}, Mathematical Finance 9 (1999), pp. 203--228.

\bibitem{bakumenko2025advan}
A.~Bakumenko, \emph{Advancing anomaly detection: Non-semantic financial data encoding with large language models}, IEEE Access 13 (2025).

\bibitem{arrieta2020explainable}
A.~Barredo Arrieta, N.~Díaz-Rodríguez, J.~Del Ser, A.~Bennetot, S.~Tabik, A.~Barbado, S.~García, S.~Gil-López, D.~Molina, R.~Benjamins, R.~Chatila, and F.~Herrera, \emph{Explainable artificial intelligence (XAI): Concepts, taxonomies, opportunities and challenges toward responsible AI}, Information Fusion 58 (2020), pp. 82--115.

\bibitem{bollerslev1986a}
T.~Bollerslev, \emph{Generalized autoregressive conditional heteroskedasticity}, Journal of Econometrics 31 (1986), pp. 307--327.

\bibitem{bouchaud2001more}
J.-P.~Bouchaud and M.~Potters, \emph{More stylized facts of financial markets: Leverage effect and downside correlations}, Physica A: Statistical Mechanics and its Applications 299 (2001), pp. 60--70.

\bibitem{breiman2001random}
L.~Breiman, \emph{Random forests}, Machine Learning 45 (2001), pp. 5--32.

\bibitem{cao2024autom}
F.~Cao and X.~Guo, \emph{Automated financial time series anomaly detection via curiosity-guided exploration and self-imitation learning}, Engineering Applications of Artificial Intelligence 133 (2024), p. 108663.

\bibitem{chandola2009anomaly}
V.~Chandola, A.~Banerjee, and V.~Kumar, \emph{Anomaly detection: A survey}, ACM Computing Surveys 41 (2009), pp. 1--58.

\bibitem{chen2016xgboost}
T.~Chen and C.~Guestrin, \emph{XGBoost: A scalable tree boosting system}, in \emph{Proceedings of the 22nd ACM SIGKDD International Conference on Knowledge Discovery and Data Mining}, 2016, pp. 785--794.

\bibitem{cont2001empirical}
R.~Cont, \emph{Empirical properties of asset returns: Stylized facts and statistical issues}, Quantitative Finance 1 (2001), pp. 223--236.

\bibitem{crepey2022anoma}
S.~Cr\'{e}pey, N.~Lehdili, N.~Madhar, and M.~Thomas, \emph{Anomaly detection on financial time series by principal component analysis and neural networks}, Algorithms 15 (2022), p. 385.

\bibitem{darban2023deep}
Z.Z.~Darban, G.I.~Webb, S.~Pan, C.C.~Aggarwal, and M.~Salehi, \emph{Deep learning for time series anomaly detection: A survey}, ACM Computing Surveys 57 (2024), pp. 1--42.

\bibitem{engle1982arch}
R.F.~Engle, \emph{Autoregressive conditional heteroscedasticity with estimates of the variance of United Kingdom inflation}, Econometrica 50 (1982), pp. 987--1007.

\bibitem{engle2004caviar}
R.F.~Engle and S.~Manganelli, \emph{CAViaR: Conditional autoregressive value at risk by regression quantiles}, Journal of Business and Economic Statistics 22 (2004), pp. 367--381.

\bibitem{fama1970efficient}
E.F.~Fama, \emph{Efficient capital markets: A review of theory and empirical work}, The Journal of Finance 25 (1970), pp. 383--417.

\bibitem{fama2015five}
E.F.~Fama and K.R.~French, \emph{A five-factor asset pricing model}, Journal of Financial Economics 116 (2015), pp. 1--22.

\bibitem{french_data}
K.R.~French, \emph{Kenneth R. French - Data Library}, (2025), \url{https://mba.tuck.dartmouth.edu/pages/faculty/ken.french/data_library.html}.

\bibitem{friedman2001greedy}
J.H.~Friedman, \emph{Greedy function approximation: A gradient boosting machine}, Annals of Statistics 29 (2001), pp. 1189--1232.

\bibitem{guidolin2011markov}
M.~Guidolin, \emph{Markov switching models in empirical finance}, Advances in Econometrics 27 (2011), pp. 1--86.

\bibitem{hamilton1989a}
J.D.~Hamilton, \emph{A new approach to the economic analysis of nonstationary time series and the business cycle}, Econometrica 57 (1989), pp. 357--384.

\bibitem{hansen1994autoregressive}
B.E.~Hansen, \emph{Autoregressive conditional density estimation}, International Economic Review 35 (1994), pp. 705--730.

\bibitem{hemrajani2026code}
P.~Hemrajani, H.~Kumar, S.~Kumar, S.~Shekhar, V.~Himthani, S.~Mahalik, and M.K.~Bohra, \emph{Explainable Regime Conditioned Mixture of Experts for Financial Anomaly Detection: Source Code} (2026), Zenodo. \url{https://doi.org/10.5281/zenodo.XXXXXXX}.

\bibitem{hemrajani2026data}
P.~Hemrajani, H.~Kumar, S.~Kumar, S.~Shekhar, V.~Himthani, S.~Mahalik, and M.K.~Bohra, \emph{Explainable Regime Conditioned Mixture of Experts for Financial Anomaly Detection: Replication Dataset} (2026), Zenodo. \url{https://doi.org/10.5281/zenodo.21257024}.

\bibitem{jacobs1991moe}
R.A.~Jacobs, M.I.~Jordan, S.J.~Nowlan, and G.E.~Hinton, \emph{Adaptive mixtures of local experts}, Neural Computation 3 (1991), pp. 79--87.

\bibitem{jorion2007value}
P.~Jorion, \emph{Value at risk: The new benchmark for managing financial risk}, McGraw-Hill, 2007.

\bibitem{ke2017lightgbm}
G.~Ke, Q.~Meng, T.~Finley, T.~Wang, W.~Chen, W.~Ma, Q.~Ye, and T.-Y.~Liu, \emph{LightGBM: A highly efficient gradient boosting decision tree}, in \emph{Advances in Neural Information Processing Systems} 30, 2017.

\bibitem{kendall1938measure}
M.G.~Kendall, \emph{A new measure of rank correlation}, Biometrika 30 (1938), pp. 81--93.

\bibitem{kupiec1995techniques}
P.H.~Kupiec, \emph{Techniques for verifying the accuracy of risk measurement models}, The Journal of Derivatives 3 (1995), pp. 73--84.

\bibitem{fan2025expla}
Z.~Li and R.~Fan, \emph{Explainable heterogeneous anomaly detection in financial networks via adaptive expert routing}, IEEE Access (2025).

\bibitem{lin2017focal}
T.-Y.~Lin et al., \emph{Focal loss for dense object detection}, in \emph{Proceedings of ICCV}, 2017, pp. 2980--2988.

\bibitem{liu2008isolation}
F.T.~Liu, K.M.~Ting, and Z.-H.~Zhou, \emph{Isolation forest}, IEEE International Conference on Data Mining (2008), pp. 413--422.

\bibitem{liu2024times}
Q.~Liu, P.~Boniol, T.~Palpanas, and J.~Paparrizos, \emph{Time-series anomaly detection: Overview and new trends}, Proceedings of the VLDB Endowment 17 (2024), pp. 4229--4232.

\bibitem{lundberg2017shap}
S.M.~Lundberg and S.-I.~Lee, \emph{A unified approach to interpreting model predictions}, in \emph{Advances in Neural Information Processing Systems} 30, 2017.

\bibitem{luo2025robus}
X.~Luo, \emph{Robust financial fraud detection via causal intervention and multi-view contrastive learning on dynamic hypergraphs}, Mathematics 13 (2025), p. 4018.

\bibitem{mandelbrot1963variation}
B.~Mandelbrot, \emph{The variation of certain speculative prices}, The Journal of Business 36 (1963), pp. 394--419.

\bibitem{mcneil2000estimation}
A.J.~McNeil and R.~Frey, \emph{Estimation of tail-related risk measures for heteroscedastic financial time series: An extreme value approach}, Journal of Empirical Finance 7 (2000), pp. 271--300.

\bibitem{mcneil2015quantitative}
A.J.~McNeil, R.~Frey, and P.~Embrechts, \emph{Quantitative Risk Management: Concepts, Techniques and Tools}, 2nd ed., Princeton University Press, 2015.

\bibitem{neloy2024a}
A.A.~Neloy and M.~Turgeon, \emph{A comprehensive study of auto-encoders for anomaly detection: Efficiency and trade-offs}, SSRN Electronic Journal (2024).

\bibitem{nelson1991egarch}
D.B.~Nelson, \emph{Conditional heteroskedasticity in asset returns: A new approach}, Econometrica 59 (1991), pp. 347--370.

\bibitem{pang2021deep}
G.~Pang, C.~Shen, L.~Cao, and A.~Van Den Hengel, \emph{Deep learning for anomaly detection: A review}, ACM Computing Surveys 54 (2021), pp. 1--38.

\bibitem{parkinson1980extreme}
M.~Parkinson, \emph{The extreme value method for estimating the variance of the rate of return}, Journal of Business 53 (1980), pp. 61--65.

\bibitem{ribeiro2016lime}
M.T.~Ribeiro, S.~Singh, and C.~Guestrin, \emph{Why should I trust you?: Explaining the predictions of any classifier}, in \emph{Proceedings of the 22nd ACM SIGKDD International Conference on Knowledge Discovery and Data Mining}, 2016, pp. 1135--1144.

\bibitem{rockafellar2000cvar}
R.T.~Rockafellar and S.~Uryasev, \emph{Optimization of conditional value-at-risk}, Journal of Risk 2 (2000), pp. 21--42.

\bibitem{scholkopf2001estimating}
B.~Sch\"{o}lkopf, J.C.~Platt, J.~Shawe-Taylor, A.J.~Smola, and R.C.~Williamson, \emph{Estimating the support of a high-dimensional distribution}, Neural Computation 13 (2001), pp. 1443--1471.

\bibitem{sharpe1966mutual}
W.F.~Sharpe, \emph{Mutual fund performance}, The Journal of Business 39 (1966), pp. 119--138.

\bibitem{song2023anoma}
A.~Song, \emph{Anomaly VAE-Transformer: A deep learning approach for anomaly detection in decentralized finance}, IEEE Access 11 (2023).

\bibitem{su2025anoma}
T.~Su, \emph{Anomaly detection and risk early warning system for financial time series based on the WaveLST-Trans}, Transactions on Engineering and Technology Application Intelligence (2025).

\bibitem{wang2024adapt}
Z.~Wang, \emph{Adaptive ensemble learning framework with SHAP-based feature optimization for financial anomaly detection}, AI and Machine Learning Research (2024).

\bibitem{wu2024anoma}
J.~Zhang, Z.~Hong, X.~Yan, and S.~Wu, \emph{Anomaly detection in financial time series based on recurrent neural networks and latent variable autoregressive models}, in \emph{Proceedings of the ACM International Conference on Multimedia}, 2024.

\bibitem{zhou2025kanad}
Q.~Zhou et al., \emph{KAN-AD: Time series anomaly detection with Kolmogorov-Arnold networks}, arXiv preprint arXiv:2411.00278 (2025).
